...
// backend/catalog/games/football-manager-2024.json - 100 real achievements sourced
// from a live ISteamUserStats/GetSchemaForGame/v2 response for appid
// 1904540 (fetched through this app's own services/steamApi.js).
// This game has no hidden achievements: all 100 ship a real, official
// Steam description, quoted verbatim below.
// difficulty/estimatedTime/missable remain curatorial judgments.
const game = getPlannerData("football-manager-2024");

test("getPlannerData('football-manager-2024') returns real planner data with 100 curated achievements", () => {

    assert.ok(game, "expected real planner data for football-manager-2024");
    assert.ok(Array.isArray(game.achievements));
    assert.strictEqual(game.achievements.length, 100);

});

test("every Football Manager 2024 achievement has a unique id from 1 to 100 and a unique apiname", () => {
...
test("every Football Manager 2024 achievement has a valid difficulty (1-5), a positive estimatedTime, and non-empty name/description", () => {
...
test("every one of the 100 Football Manager 2024 achievement name+description pairs matches the live GetSchemaForGame response", () => {
