...
// backend/catalog/games/intrusion-2.json - 14 real achievements sourced
// from a live ISteamUserStats/GetSchemaForGame/v2 response for appid
// 214970 (fetched through this app's own services/steamApi.js). None are
// hidden; every description is Steam's own real text.
// difficulty/estimatedTime/missable remain curatorial judgments.
const game = getPlannerData("intrusion-2");

test("getPlannerData('intrusion-2') returns real planner data with 14 curated achievements", () => {

    assert.ok(game, "expected real planner data for intrusion-2");
    assert.ok(Array.isArray(game.achievements));
    assert.strictEqual(game.achievements.length, 14);

});

test("every Intrusion 2 achievement has a unique id from 1 to 14 and a unique apiname", () => {
...
test("every Intrusion 2 achievement has a valid difficulty (1-5), a positive estimatedTime, and non-empty name/description", () => {
...
test("every one of the 14 Intrusion 2 achievement name+description pairs matches the catalog data", () => {
