...
// backend/catalog/games/mafia-2-classic.json - 67 real achievements sourced
// from a live ISteamUserStats/GetSchemaForGame/v2 response for appid
// 50130 (fetched through this app's own services/steamApi.js). None are
// hidden; every description is Steam's own real text.
// difficulty/estimatedTime/missable remain curatorial judgments.
const game = getPlannerData("mafia-2-classic");

test("getPlannerData('mafia-2-classic') returns real planner data with 67 curated achievements", () => {

    assert.ok(game, "expected real planner data for mafia-2-classic");
    assert.ok(Array.isArray(game.achievements));
    assert.strictEqual(game.achievements.length, 67);

});

test("every Mafia II (Classic) achievement has a unique id from 1 to 67 and a unique apiname", () => {
...
test("every Mafia II (Classic) achievement has a valid difficulty (1-5), a positive estimatedTime, and non-empty name/description", () => {
...
test("every one of the 67 Mafia II (Classic) achievement name+description pairs matches the catalog data", () => {
