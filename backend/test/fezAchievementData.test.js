...
// backend/catalog/games/fez.json - 12 real achievements sourced
// from a live ISteamUserStats/GetSchemaForGame/v2 response for appid
// 224760 (fetched through this app's own services/steamApi.js). None are
// hidden; every description is Steam's own real text.
// difficulty/estimatedTime/missable remain curatorial judgments.
const game = getPlannerData("fez");

test("getPlannerData('fez') returns real planner data with 12 curated achievements", () => {

    assert.ok(game, "expected real planner data for fez");
    assert.ok(Array.isArray(game.achievements));
    assert.strictEqual(game.achievements.length, 12);

});

test("every FEZ achievement has a unique id from 1 to 12 and a unique apiname", () => {
...
test("every FEZ achievement has a valid difficulty (1-5), a positive estimatedTime, and non-empty name/description", () => {
...
test("every one of the 12 FEZ achievement name+description pairs matches the catalog data", () => {
    ...
    const officialAchievements = [
        ...
        ["Haikus not epics", "Find the Tome artifact."],
